...
- Create a new bucket named `pet.myway.lifecycle` with following properties. Bucket name has to be unique.
...
- Click the S3 bucket `pet.myway.lifecycle` and upload following files.
...
- Create a bucket of `source.replica.myway` with following properties,
...
- Click the S3 bucket `source.replica.myway` and upload following files.
...
- Create a new bucket named `destination.cross.region.replica.myway` with following properties.
    ...
    "Statement": [
        {
            ...
            "Effect": "Allow",
            "Resource": [
                "arn:aws:s3:::source.replica.myway(change me)",
                "arn:aws:s3:::source.replica.myway(change me)/*"
            ]
        },
        {
            ...
            "Effect": "Allow",
            "Resource": "arn:aws:s3:::destination.cross.region.replica.myway(change me)/*"
        }
    ]
}
```
...
- Go to `source.replica.myway` bucket and upload `index.html` and `cat.jpg` again.

- Go to `destination.cross.region.replica.myway` bucket, copy `Endpoint` and paste to browser.
...
d. Destination                      : 
   - Choose a bucket in this account
   - Bucket name : destination.cross.region.replica.myway
...
- Go to `source.replica.myway` bucket and upload `outside_folder.txt`.

- Go to `destination.cross.region.replica.myway` bucket and show that the`outside_folder.txt` is not replicated in the bucket.

- This time go to `source.replica.myway` bucket >> `kitten` folder and upload `inside_kitten_folder.txt`.

- Go to `destination.cross.region.replica.myway` bucket show that `kitten` folder has been replicated automatically due to replication rule and inside the folder there is `inside_kitten_folder.txt file`.
...
- Go to `source.replica.myway` bucket and direct upload `tag_file.txt`.

- Go to `destination.cross.region.replica.myway` bucket and show that the `tag_file.txt` is not replicated in the bucket.

- This time go to `source.myway` bucket, and upload the `tag_file.txt` file with tag as `key:name` and `value:cat`;
...
- Go to `destination.cross.region.replica.myway` bucket show that the `tag_file.txt` file is replicated.
- Well done...!!!
